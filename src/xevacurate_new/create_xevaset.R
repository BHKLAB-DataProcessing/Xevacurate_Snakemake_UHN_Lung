...
rna_fdata_csv <- file.path(OUT_DIR, "rna_fdata.csv")
...
safe_set_response <- function(xset, measure) {
  tryCatch({
    setResponse(xset, res.measure = measure)
  }, error = function(e) {
    message("setResponse('", measure, "') failed: ", conditionMessage(e))
    xset
  }, warning = function(w) {
...
for (m in c("slope", "AUC", "angle", "abc", "mRECIST", "TGI", "lmm")) {
  xeva.set <- safe_set_response(xeva.set, m)
}
...
cat("Wrote RNA feature data to:", rna_fdata_csv, "\n")
